...
build:
	@go build -ldflags "-s -w" -o $(DST) cmd/main.go

build-windows-amd64:
	@GOOS=windows GOARCH=amd64 go build -ldflags "-s -w" -o bin/windows/SnakeGame.exe cmd/main.go
...
build-darwin-arm64:
	@GOOS=darwin GOARCH=arm64 go build -ldflags "-s -w" -o bin/macOS/SnakeGame cmd/main.go
